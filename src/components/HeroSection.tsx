...
//   useEffect(() => {
//     const targetDate = new Date('2025-08-23T08:00:00');
...
import React, { useEffect, useState } from 'react';
import { motion } from 'framer-motion';
import { Link } from 'react-scroll'; // for smooth scrolling to Prizes component

const HeroSection: React.FC = () => {
  const [showGlitch, setShowGlitch] = useState(false);

  useEffect(() => {
    const startGlitch = setTimeout(() => setShowGlitch(true), 1000);
    const stopGlitch = setTimeout(() => setShowGlitch(false), 6000);
      ...
      <div className="min-h-screen max-w-6xl w-full h-full flex flex-col justify-end pb-12 lg:pb-4 md:pb-12 relative z-10">
        ...
        <div className="text-center">
          ...
          <motion.div
            ...
            transition={{ delay: 1, duration: 0.8 }}
          >
            <motion.div className="mb-1 text-[#ff2a93]">PREPARE FOR REVELATION</motion.div>
            <motion.div className="mb-1 text-white">HEIGHTEN YOUR SENSES</motion.div>
            <motion.div className="text-[#2ef2ff]">THE UNVEILING NEARS</motion.div>
          </motion.div>
          ...
          {/* SUCCESSFULLY CONCLUDED MESSAGE */}
          <div className="flex justify-center">
            <div className="bg-black bg-opacity-50 backdrop-blur-sm p-6 rounded-2xl w-full max-w-2xl mx-4 sm:mx-0 shadow-xl">
              <h3 className="text-2xl sm:text-3xl md:text-4xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-[#ff2a93] via-[#2ef2ff] to-[#7a2aff] text-center mb-3">
                🎉 Genesiz25 Successfully Concluded! 🎉
              </h3>
              <p className="text-gray-300 text-sm sm:text-base md:text-lg text-center mb-6">
                Thank you to all participants for being part of this amazing journey!
              </p>

              {/* See Winners Button */}
              <div className="flex justify-center">
                <Link
                  to="prizes" // id of your Prizes component
                  smooth={true}
                  duration={600}
                >
                  <motion.button
                    className="px-8 py-3 bg-gradient-to-r from-[#ff2a93] to-[#7a2aff] text-white font-bold rounded-full text-lg shadow-lg hover:shadow-[0_0_20px_rgba(255,42,147,0.8)] transition-all"
